...
#Load TFLite model
model_path ="RK-GRU16-LR-Dense256-gelu-fs=36-cls=5.tf"
# Load the model
...
new_width = 680
new_height = 480
actions = ['donkey','listen', 'look', 'mouse', 'shhh']
trained_frame = 36
...
while cap.isOpened():
  ...
  if ret == False:
      ...
      sys.exit(0)
  #Make detections
  predict_start = time.time()  # Start time for model prediction
  ...
  if len(sequence) == trained_frame:
    ...
    predict_end = time.time()  # End time for model prediction
